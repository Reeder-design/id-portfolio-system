...
def page_library():
    flash(
        "The legacy structured-to-HTML page editor is retired. Use the current page-copy editor instead.",
        "info",
    )
...
def save_page(page_id: str):
    flash(
        "The legacy structured-to-HTML save path is retired. No portfolio files were changed.",
        "info",
    )
    return redirect(url_for("site_content.v2_page_editor", page_id=page_id), code=303)
