(function(){
  ...
  const roleData={
    admin:{label:'Administration',title:'Build reusable structures with controlled flexibility.',text:'I design audience architecture, roles, catalogs, learning plans, certification rules, visibility, prerequisites, ownership, and exception paths so the learner route is intentional and administrators can adapt it without creating manual workarounds.',practice:['Audience architecture','Learning-plan logic','Certification rules','Permissions + ownership'],evidence:'The configuration produces the expected learner population, access rules, assignment state, administrative ownership, and a clear path for exceptions.',map:['Identity','Rules','Route']},
    delivery:{label:'Delivery',title:'Treat publishing as release management, not an upload step.',text:'I validate package behavior, metadata, catalog placement, prerequisites, learning-plan relationships, completion rules, version replacement, and learner-facing instructions before release. The goal is to preserve the intended learner route and historical records as content changes.',practice:['Package behavior','Version control','Prerequisites','Release validation'],evidence:'The right package launches in the right context, records the intended status, and can be updated without breaking the learner history.',map:['Package','Validate','Release']},
    support:{label:'Support',title:'Turn learner friction into a better operating system.',text:'I troubleshoot access, navigation, completion, certification, and assignment issues; maintain reusable guidance and support responses; document recurring failures; and route patterns back into configuration, QA, documentation, or escalation paths.',practice:['Issue triage','Learner guidance','Support documentation','Pattern analysis'],evidence:'The immediate issue is resolved, the root cause is documented, and repeated friction becomes an improvement signal instead of recurring manual work.',map:['Signal','Root cause','Improve']}
  };
