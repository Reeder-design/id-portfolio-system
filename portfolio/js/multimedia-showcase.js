(() => {
  ...
  const renderWorkflow = () => {
    ...
    ['One','Two','Three'].forEach((item, index) => {
      ...
      setText('workflowTrack' + item, format.tracks[index]);
    });
    setText('workflowPreviewName', data.preview);
    setText('workflowPreviewStatus', stage.preview);
    setText('workflowOverlayLabel', data.overlay);
    setText('workflowOverlayDetail', data.detail);
    ...
    setText('workflowTrackTitle', selectedWorkflowStage === 'frame' ? 'PRODUCTION PLAN' : selectedWorkflowStage === 'release' ? 'DELIVERY PACKAGE' : 'WORKING LAYERS');
    setText('workflowTrackStatus', stage.editor);
    ...
    if (motion) {
      ...
      motion.setAttribute('aria-label', 'Illustrative ' + format.name.toLowerCase() + ' production editor, ' + stage.name.toLowerCase() + ' stage. ' + data.overlay + ': ' + data.detail + '. ' + data.next);
