...
def main() -> int:
    errors: list[str] = []
    files = [SERVICE, ROUTES, WORKSPACE, BRIEF, AI_WORKSPACE, DASHBOARD, APP]
    for path in files:
    ...
    for target in forbidden:
        require(target not in service + routes, f"Create Content foundation must remain planning-only: found {target}", errors)
    ...
    require("Idea → Brief → AI Plan" in brief, "Content Brief must expose the planning-first workflow.", errors)
